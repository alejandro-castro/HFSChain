...
time_end   = results.time_end#"02:00:00"#
...
def CalculateMoments(oldspec, oldfreq, n0, nicoh = None, graph = None, smooth = None, type1 = None, fwindow = None, snrth = None, dc = None, aliasing = None, oldfd = None, wwauto = None):
	...
	for ind in range(oldspec.shape[1]):
		...
		#Filtrado del espectro, se puede mejorar y hacerlo mas rapido para encontrar ambos puntos en el olfreq y asi no iterar por todos los perfiles
		#print oldspec.shape, "gg", oldfreq.shape, "gg"
		for i in range(oldspec.shape[0]): #Perfiles
			if (oldfreq[i] < fd - 3*w) or (oldfreq[i] > fd + 3*w):
				oldspec[i][ind] /= 10000
...
for each_ch in glob.glob(doypath+"sp%s1_f%s/"%(code,freqidx)):
	...
	spc_db = numpy.zeros((1000,colmsnumber//step,3))
	...
	for filename in numpy.arange(0,Ntimespec//step):
		...
		if filedatatime>=initTimeXaxisValue and filedatatime<=lastdatatime:
			...
			# for i in range(data_a_matrix.shape[1]):
			# 	for j in range(data_a_matrix.shape[0]):
			# 		print i,j, data_a_matrix[j][i]
			#print L, linearfactor, N, i0l, i0h, im
			colm=numpy.zeros([N,3],dtype=numpy.float32)
			for ri in numpy.arange(N):
				colm[ri,2]= numpy.sum(data_a_matrix[i0h:L,ri])/(L-i0h) #+ numpy.sum(data_a_matrix[i0h:L,ri])
				colm[ri,1]= numpy.sum(data_a_matrix[i0l:i0h,ri])/(i0h-i0l)
				#print "gg", data_a_matrix[i0l:im,ri]
				colm[ri,0]= numpy.sum(data_a_matrix[:i0l,ri])/i0l
			...
			for i in range(r2):
				for j in range(npy):
					...
					if snr[j]>500*0.01: #and image[i][j]> 1e-11*(i0h-i0l):#(image[i][j]> 1e-13) and (snr[j]>100):#snr[j]>0.1):
