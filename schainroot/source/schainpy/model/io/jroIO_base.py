...
class JRODataWriter(JRODataIO):
	...
	aditional = ''#This attribute helps you add a string to the filename of data to be written
	...
	def setNextFile(self):
		...
		file = '%s%4.4d%3.3d%3.3d%s' % (self.optchar,
										...
										ext )
		if self.standard:
			file = '%s%4.4d%3.3d%s%s%s%s%1.1d%s' % (self.optchar, timeTuple.tm_year, timeTuple.tm_yday,self.c_station_o,self.c_freq,self.c_cod_Tx,self.c_ch, setFile, ext )
	...
	def setup(self, dataOut, path, blocksPerFile, profilesPerBlock=64, set=0, ext=None,datatype=4,standard=False, c_station_o=None,c_freq=None,c_cod_Tx=None,c_ch=None):
		"""
