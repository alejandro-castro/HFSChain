...
#        self.updateDataHeader()
    ...
    def getBasicHeader(self):
        ...
        self.dataOut.timeZone = self.basicHeaderObj.timeZone*60
...
#            self.setFile = -1 #inicializo mi contador de seteo
            ...
            for thisFile in filesList:
                ...
                if  number >=(self.rank+1)*(720/self.cpu) or number <self.rank*(720/self.cpu):
                    continue
                rank_validFilelist.append(thisFile)
                
                #print "rank",self.rank,"size",self.cpu, rank_validFilelist                    #P2015269036.
            if len(rank_validFilelist)>0:
                rank_validFilelist = sorted(rank_validFilelist,key=str.lower)
                filen=rank_validFilelist[-1]
                if isNumber(filen[8:11]):
                    setFile= int(filen[8:11])
                else:
                    setFile = self.rank*(720/self.cpu) - 1
...
#        setFile = self.setFile
    ...
    def setup(self, dataOut, path, blocksPerFile, profilesPerBlock=64, set=0, ext=None,rank=0,cpu=1):
        """
