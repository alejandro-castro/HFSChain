...
def createFigure(id, wintitle, width, height, facecolor="w", show=True):
	...
	DPI = matplotlib.pyplot.rcParams["figure.dpi"]
	width_inch = width*1.0/DPI
	heigh_inch = height*1.0/DPI
	##Creating the image
	fig = matplotlib.pyplot.figure(num=id, figsize=(width_inch, heigh_inch),facecolor=facecolor)
...
def createPcolor(ax, x, y, z, xmin, xmax, ymin, ymax, zmin, zmax,
				 ...
				 colormap='jet',cblabel='', cbsize="10%",
				 XAxisAsTime=False):
	...
	#TODO : somethings is wrong..
	#cb =  matplotlib.pyplot.colorbar(imesh)
