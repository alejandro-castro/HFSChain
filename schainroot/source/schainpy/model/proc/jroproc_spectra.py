...
class SpectraProc(ProcessingUnit):
	...
	def __updateObjFromInput(self):
		attributes = ["timeZone", "dstFlag", "errorCount", "useLocalTime", "channelList", "heightList",
		"nBaud", "nCode", "code", "flagTimeBlock", "flagDecodeData", "flagDeflipData", "nCohInt", "frequency",
		"realtime", "azimuth", "zenith","last_block", "flagLastFile"]
		...
		self.dataOut.nIncohInt = 1 # 6 para un minuto y 1 para 10 segundos(campana).
	...
	def __getPapu(self):
		fft_volt = self.buffer
		cspc = numpy.zeros((self.dataOut.nFFTPoints, self.dataOut.nHeights), dtype='complex')
		cspc[:,:]=self.buffercspec
		...
		if (cspc == None).any():
			raise ValueError, "No debio entrar aqui"
			if self.dataOut.pairsList != None:
				#calculo de cross-spectra
				cspc = numpy.zeros((self.dataOut.nFFTPoints, self.dataOut.nHeights), dtype='complex')
				cspc[:,:] = fft_volt[pair[0],:,:] * numpy.conjugate(fft_volt[pair[1],:,:])
				blocksize += cspc.size
		...
		self.dataOut.Image = self.bufferImage
	...
	def run(self, nProfiles=None, nFFTPoints=None, pairsList=[], ippFactor=1,noiseMode = None):
		...
		if self.dataIn.type == "Voltage":
			...
			#self.dataOut.noiseMode = noiseMode
			if type(self.buffer) == types.NoneType:
				self.buffer = numpy.zeros((self.dataIn.nChannels,
											nProfiles,
											self.dataIn.nHeights),
											dtype='complex')
			#TODO 2018 : buffercspec and buffer image are not voltaje type, we should move to another class
			if type(self.buffercspec) == types.NoneType:
				self.buffercspec = numpy.zeros((nProfiles,
											self.dataIn.nHeights),
											dtype='complex')

			if type(self.bufferImage) == types.NoneType:
				self.bufferImage = numpy.zeros((self.dataIn.nHeights,
											3,
											self.dataIn.nChannels),
											dtype='float')

				self.id_min = 0
				self.id_max = self.dataIn.data.shape[1]

			#print self.buffer.shape, self.buffercspec.shape, self.bufferImage.shape, self.id_max
			if len(self.dataIn.data.shape) == 2:
				#print self.profIndex,"data", self.dataIn.data.copy(), self.buffer
				self.buffer[:,self.profIndex,:] = self.dataIn.data.copy()
				self.buffercspec[self.profIndex,:]= self.dataIn.data_cspec.copy()
				self.bufferImage = self.dataIn.Image.copy() #this data comes from dataout.Image

				self.profIndex += 1
			else:
				#Nunca deberia entrar aqui
				raise ValueError, "Nunca debio entrar aqui"
			...
			if self.profIndex == nProfiles:
				self.nspectrogram += 1
				print "Fin de analisis de espectrograma N %s" % self.nspectrogram
	...
	def removeDC(self, mode = 2):
		...
		if mode == 2:
			...
			if jcspectraExist:
				yy = jcspectra[ind_vel,:]
				jcspectra[freq_dc,:] = numpy.dot(xx_aux,yy)
	...
	# 	#Alejandro May 2019: No funciona por el momento porque se cambio el formato de cspec
	#
	# 	jspectra = self.dataOut.data_spc
	# 	jcspectra = self.dataOut.data_cspc
	# 	jnoise = self.dataOut.getNoise(1)
	...
	# 	if (jnoise.shape[0] < num_channel or numpy.isnan(jnoise).any()):
	# 		jnoise = numpy.nan
	# 		noise_exist = False
	# 	else:
	# 		noise_exist = True
	...
	# 		#print "here", junkspc_interf
	#
	# 		if noise_exist:
	# 		#	tmp_noise = jnoise[ich] / num_prof
	# 			tmp_noise = jnoise[ich]#[aux].sum(axis=0)
	# 		#print numpy.repeat(tmp_noise,num_prof).reshape(len(aux), num_prof)
	# 		junkspc_interf = junkspc_interf - tmp_noise #numpy.repeat(tmp_noise,num_prof).reshape(len(aux), num_prof)
	...
	def removeInterference(self,  interf = 2,hei_interf = None, nhei_interf = None, offhei_interf = None):
		...
		#Remocion de Interferencia en el Cross Spectra
		if (jcspectra == None).any(): return jspectra, jcspectra
		jcspectra = jcspectra.reshape(num_prof, num_hei)
		for ip in range(1):

			#-------------------------------------------

			cspower = numpy.abs(jcspectra[mask_prof,:])
			cspower = cspower[:,hei_interf]
			cspower = cspower.sum(axis = 0)

			cspsort = cspower.ravel().argsort()
			junkcspc_interf = jcspectra[:,hei_interf[cspsort[range(offhei_interf, nhei_interf + offhei_interf)]]]
			#junkcspc_interf = junkcspc_interf.transpose()
			jcspc_interf = junkcspc_interf.sum(axis = 1)/nhei_interf
			ind = numpy.abs(jcspc_interf[mask_prof]).ravel().argsort()
			...
			#Removiendo la Interferencia
			jcspectra[:,ind_hei] = jcspectra[:,ind_hei] - numpy.repeat(jcspc_interf,num_hei).reshape(num_prof, num_hei)
			...
			ind = (ind + maxid + num_mask_prof)%num_mask_prof
			yy = jcspectra[mask_prof[ind],:]
			jcspectra[mask_prof[maxid],:] = numpy.dot(yy.transpose(),xx)
		#-------------------------------------------

		#Guardar Resultados
		self.dataOut.data_spc = jspectra
		self.dataOut.data_cspc = jcspectra
		#print "cross spectra",jcspectra[:10]
		return 1
	...
	def getNoise(self, minHei=None, maxHei=None, minVel=None, maxVel=None):
		...
		print "nIncohInt",self.dataOut.nIncohInt
